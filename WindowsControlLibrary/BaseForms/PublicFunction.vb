...
Public Module PublicFunction
    ...
    'Public Sub LoadCombobox_multicolum(ByVal muticbo As Janus.Windows.GridEX.EditControls.MultiColumnCombo, ByVal tb As DataTable, ByVal Text As String, ByVal Value As String, Optional ByVal ShowAll As Boolean = False)
    '    If tb.Rows.Count <= 0 Then
    '        Exit Sub
    '    End If
    '    Dim i As Integer
    '    Dim item As UIComboBoxItem
    '    'If (ShowAll) Then muticbo.Controls.Items.Add("--All--", String.Empty)
    '    If (tb.Rows.Count = 0) Then Exit Sub

    '    muticbo.DataSource = tb
    '    muticbo.ValueMember = Value
    '    muticbo.DisplayMember = Text
    ...
    '    Next
    'End Sub
    'Public Sub LoadYear(ByVal cbo As UIComboBox)
    '    Dim i As Integer
    '    For i = 2006 To Date.Now.Year
    '        cbo.Items.Add(i.ToString, i)
    '    Next
    '    cbo.SelectedValue = Date.Now.Year
    'End Sub

    'Public Sub LoadMonth(ByVal cbo As UIComboBox)

    '    Dim i As Integer
    '    For i = 1 To 12
    '        cbo.Items.Add(Microsoft.VisualBasic.Right("0" + i.ToString, 2), i)

    '    Next
    '    cbo.SelectedValue = Date.Now.Month
    'End Sub
...
#Region "XuLyNgonNgu ContextMenu"
    ...
    Public Sub exportLangText(ByRef langDic As DataTable, ByVal ctrls As System.Windows.Forms.Control.ControlCollection, ByVal parentText As String)
        ...
        For Each myControl As Control In ctrls
            If (TypeOf myControl Is Panel Or TypeOf myControl Is GroupBox Or TypeOf myControl Is TabControl Or TypeOf myControl Is TabPage Or TypeOf myControl Is XtraTabControl) Then
                If TypeOf myControl Is XtraTabControl Then
                    For Each tabC As XtraTabPage In CType(myControl, XtraTabControl).TabPages
                        langDic.Rows.Add(New Object() {String.Format("{0}.{1}.{2}", parentText, myControl.Name, tabC.Name), tabC.Text})
                    Next
                Else
            ...
            ElseIf (TypeOf myControl Is GridControl) Then
                ...
                If Not IsNothing(myGridView) Then
                    ...
                    For Each col As Columns.GridColumn In myGridView.Columns
                        ' KiemTra
                        getTextFromColumnTitle(langDic, col, parentText & "." & myGridView.Name)
                    Next
                End If
            End If
    ...
    Public Sub changeLangForm(ByVal langDic As DataTable, ByVal ctrls As System.Windows.Forms.Control.ControlCollection, ByVal parentText As String, Optional ByVal onlyGrid As Boolean = 0)
        'DuyetQuaCacControls
        For Each myControl As Control In ctrls
            If (TypeOf myControl Is Panel Or TypeOf myControl Is GroupBox Or TypeOf myControl Is TabControl Or TypeOf myControl Is TabPage Or TypeOf myControl Is RadioButton Or TypeOf myControl Is CheckBox Or TypeOf myControl Is XtraTabControl Or TypeOf myControl Is XtraTabPage) Then
                If onlyGrid = 0 Then
                    If TypeOf myControl Is XtraTabControl Then
                        For Each tabC As XtraTabPage In CType(myControl, XtraTabControl).TabPages
                            aText = String.Format("{0}.{1}", "General", tabC.Name)
                    ...
                    End If
                End If
                changeLangForm(langDic, myControl.Controls, parentText, onlyGrid)
            ElseIf (TypeOf myControl Is GridControl) Then
                'DuyetQuaColumns
                Dim myGridControl As GridControl = CType(myControl, GridControl)
                Dim myGridView As GridView = myGridControl.FocusedView
            ...
            If onlyGrid = 0 Then
                If TypeOf myControl Is Label Or TypeOf myControl Is Button _
                    Or TypeOf myControl Is DevExpress.XtraEditors.SimpleButton Then
                    setTextToControl(langDic, myControl, parentText)
                ElseIf TypeOf myControl Is DevExpress.XtraEditors.SimpleButton Then
                    setTextToContextMenu(langDic, myControl, parentText)
                End If
            End If
        Next
...
#Region "XuLyNgonNgu Grid"
    ' Start ##############
    Private Sub getTextFromColumnTitle(ByRef langDic As DataTable, ByVal myControl As GridColumn, ByVal parentText As String)
    ...
    'Private Sub setColumnTitle(ByVal langDic As DataTable, ByVal myControl As GridEXColumn, ByVal parentText As String)
...
#Region "XuLyNgonNgu Control"
    ...
    Public Sub setTextToControl(ByVal langDic As DataTable, ByVal myControl As Control, ByVal parentText As String)
        ' Ghi DisplayText
        aText = getKey(myControl, parentText)
        If TypeOf myControl Is Label Or TypeOf myControl Is Button Or TypeOf myControl Is CheckBox Or TypeOf myControl Is RadioButton Then
            If TypeOf myControl Is CheckBox Or TypeOf myControl Is RadioButton Then
...
#End Region
    ...
    Public Function AdjustCaption(ByVal ctrl As Control) As String
        ...
        If TypeOf ctrl Is CheckBox Or TypeOf ctrl Is RadioButton Then
            Return text
        Else
            If (text.StartsWith("lbl") Or text.StartsWith("btn")) Then
                Return text.Substring(3)
            ElseIf (text.StartsWith("lb") Or text.StartsWith("bt") Or TypeOf ctrl Is GroupBox) Then
    ...
    'Public Function AdjustCaptionCol(ByVal ctrl As GridEXColumn) As String
    '    Dim text As String = ctrl.Key
    '    If (Not IsNothing(ctrl.DataMember)) Then
    '        text = ctrl.DataMember
    '    End If
    '    Return text
    'End Function
