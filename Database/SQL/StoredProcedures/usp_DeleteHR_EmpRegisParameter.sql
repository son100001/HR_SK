CREATE PROCEDURE [dbo].[usp_DeleteHR_EmpRegisParameter]
...
BEGIN
	...
	set @ThongBao=''
	select @Block_Date=Block_Date from HR_Khoa where [TableName]='HR_EmpRegisParameter' and Block_User=@UserName
	...
	if @ThongBao='' begin
		if @Block_Date<@fromdate or @Block_Date is null begin
			delete HR_EmpRegisParameter where Employee_ID=@Employee_ID and fromdate=@fromdate and Parameter=@Parameter
		end else begin
