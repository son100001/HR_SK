CREATE PROCEDURE [dbo].[usp_InsertUpdateHR_DanhSachNguoiPhuThuoc]
...
BEGIN
	...
	if isnull(@ThongBao,'')='' begin
		...
		if exists(select * from HR_DanhSachNguoiPhuThuoc where (Employee_ID=@EmpID and BirthDate=@BirthDate and REPLACE([dbo].[non_unicode_convert](RelatedName),' ','')=@NonUNCRelatedName) or ID=isnull(@ID,0)) begin
			...
			insert into HR_DanhSachNguoiPhuThuoc (Employee_ID,RelatedName,Tel,RelatedType,[Address],Occupation,BirthDate,Sex,GKS_So,GKS_QuyenSo,GKS_TinhTP,GKS_QuanHuyen,GKS_PhuongXa,MaSoThue,QuocTich,ID_Number,ID_date,ID_place,DependFromMonth,DependToMonth,isDaNopGiay,Remark,UserName,InsertDate)
			values(@EmpID,@RelatedName,@Tel,@RelatedType,@Address,@Occupation,@BirthDate,@Sex,@GKS_So,@GKS_QuyenSo,@GKS_TinhTP,@GKS_QuanHuyen,@GKS_PhuongXa,@MaSoThue,@QuocTich,@ID_Number,@ID_date,@ID_place,@DependFromMonth,@DependToMonth,@isDaNopGiay,@Remark,@UserName,GETDATE())
		end
	end
	select @ID=ID from HR_DanhSachNguoiPhuThuoc where Employee_ID=@EmpID and BirthDate=@BirthDate and REPLACE([dbo].[non_unicode_convert](RelatedName),' ','')=@NonUNCRelatedName
