...
create PROCEDURE [dbo].[usp_DeleteHR_LeaveRequestGoOut]
...
BEGIN
	...
	FROM HR_Khoa
	WHERE [TableName]='HR_LeaveRequestGoOut' and Block_User=@UserName

	--set @ThongBao=@ThongBao+[dbo].[udf_KiemTraKhoaDuLieuNguoiThoiViec](@Employee_ID,@UserName)
	
	IF @ThongBao=''
	BEGIN
		if @Block_Date<@TimeDate or @Block_Date is null 
		BEGIN
			if @ID is not NULL
			BEGIN
				delete HR_LeaveRequestGoOut
				WHERE ID = @ID
			END
			ELSE begin
				delete HR_LeaveRequestGoOut
				WHERE Employee_ID=@Employee_ID and TimeDate=@TimeDate and LeaveType_ID=@LeaveType_ID
			end
		END
		ELSE begin
