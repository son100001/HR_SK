CREATE PROCEDURE [dbo].[sp_BangTayNgheNhanVien]
...
BEGIN
	...
	if @TypeOfReport=1 begin
		...
		,empl.starteddate,btn.*
		from HR_BacTayNgheNhanVien btn
		INNER join
		[dbo].[udf_EmployeeFilter](@LAN,@fact,@dept,@sect,@team,@pos,@posc,@emp,isnull(dateadd(month,1,@NgayDauThang)-1,getdate())) empl
		on btn.Employee_ID=empl.Employee_ID and empl.Employee_ID is not null 
	end
END
