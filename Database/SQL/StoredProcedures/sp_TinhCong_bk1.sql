...
create PROCEDURE [dbo].[sp_TinhCong_bk1]
	...
	--exec [dbo].[sp_TinhCong] '2025-07-31','2025-07-31','admin',null,null,null,null,null,null,'C16357'
	@fromdate datetime,
	@todate datetime,
...
BEGIN
	...
	if isnull(@ThongBao,'')='' begin
		...
		DECLARE @IsSunday bit, @IsHol50 bit, @IsHol99 bit;
		DECLARE @OldIsSunday bit, @OldIsHol50 bit, @OldIsHol99 bit;
		...
		IF OBJECT_ID('tempdb..#WT_TMP') IS NOT NULL DROP TABLE #WT_TMP;
		CREATE TABLE #WT_TMP
		(
		...
		-- Index đơn giản đủ dùng cho lọc/đếm
		CREATE NONCLUSTERED INDEX IX_#WT_TMP ON #WT_TMP(MaCong) INCLUDE (wt);
		
		/* ==== Nguồn dữ liệu cho CURSOR: #SRC (đã lọc & có cờ CN/lễ) ==== */
		IF OBJECT_ID('tempdb..#SRC') IS NOT NULL DROP TABLE #SRC;

		;WITH Cal AS (
			/* Lấy tất cả ngày có trong udf_TinhCong để tính cờ CN/lễ 1 lần */
			SELECT CAST(tc.TimeDate AS date) AS Ngay
			FROM dbo.udf_TinhCong(@fromdate,@todate,@SoNgaySauKhiMangThaiDuocHuongCheDoThaiSan,
								  @fact,@Dept,@Sect,@Team,@Pos,@PosC,@Emp) tc
			GROUP BY CAST(tc.TimeDate AS date)
		)
		, CalFlag AS (
			SELECT  c.Ngay,
					/* Lưu ý DATEFIRST trên server; nếu cần, SET DATEFIRST 7 để CN=1 */
					CASE WHEN DATEPART(WEEKDAY, c.Ngay) = 1 THEN 1 ELSE 0 END AS IsSunday,
					/* Cờ lễ theo từng loại */
					CASE WHEN EXISTS (SELECT 1 FROM SmartBooks_HolidaysPlan h 
									  WHERE h.H_date = c.Ngay AND h.TypeOfLeave = '50') THEN 1 ELSE 0 END AS IsHol50,
					CASE WHEN EXISTS (SELECT 1 FROM SmartBooks_HolidaysPlan h 
									  WHERE h.H_date = c.Ngay AND h.TypeOfLeave = '99') THEN 1 ELSE 0 END AS IsHol99
			FROM Cal c
		)
		SELECT
			tc.Employee_ID,
			...
			tc.ShiftName,
			tc.maxovertime,
			ISNULL(tc.maxovertimeB,0)      AS MaxOverTimeBefore,
			ISNULL(tc.MaxOverTimeHol,0)    AS MaxOverTimeHol,
			ISNULL(tc.maxovertimeLunch,0)  AS MaxOverTimeLunch,
			tc.CheDo,
			tc.HolSunTypeOfOT,
			cast(year(tc.TimeDate) as nvarchar) + '-' + cast(month(tc.TimeDate) as nvarchar) + '-' + cast(day(tc.TimeDate) as nvarchar) + ' ' + cast(DATEPART(hh,s.FromTime) as nvarchar) + ':' +cast(datepart(mi,s.FromTime) as nvarchar) + ':' + cast(DATEPART(ss,s.FromTime) as nvarchar) as FromTime, 
			cast(year(tc.TimeDate) as nvarchar) + '-' + cast(month(tc.TimeDate) as nvarchar) + '-' + cast(day(tc.TimeDate) as nvarchar) + ' ' + cast(DATEPART(hh,s.ToTime) as nvarchar) + ':' +cast(datepart(mi,s.ToTime) as nvarchar) + ':' + cast(DATEPART(ss,s.ToTime) as nvarchar) as ToTime,
			cast(year(tc.TimeDate) as nvarchar) + '-' + cast(month(tc.TimeDate) as nvarchar) + '-' + cast(day(tc.TimeDate) as nvarchar) + ' ' + cast(DATEPART(hh,s.RestTimeFrom) as nvarchar) + ':' +cast(datepart(mi,s.RestTimeFrom) as nvarchar) + ':' + cast(DATEPART(ss,s.RestTimeFrom) as nvarchar) as RestTimeFrom, 
			cast(year(tc.TimeDate) as nvarchar) + '-' + cast(month(tc.TimeDate) as nvarchar) + '-' + cast(day(tc.TimeDate) as nvarchar) + ' ' + cast(DATEPART(hh,s.RestTimeTo) as nvarchar) + ':' +cast(datepart(mi,s.RestTimeTo) as nvarchar) + ':' + cast(DATEPART(ss,s.RestTimeTo) as nvarchar) as RestTimeTo,
			s.AllowLateIn, s.AllowEarlyOut, s.MinMinute,
			ef.StartedDate,
			gout.TimeIn                    AS GoutTimeIn,
			gout.TimeOut_                  AS GoutTimeOut,
			gout.LeaveType_ID              AS GoutLeaveType,
			ISNULL(erp.remark, ef.ChoPhepSoPhutMuon) AS ChoPhepSoPhutMuon,
			CAST(cf.IsSunday AS bit)       AS IsSunday,
			CAST(cf.IsHol50  AS bit)       AS IsHol50,
			CAST(cf.IsHol99  AS bit)       AS IsHol99

		INTO #SRC
		FROM dbo.udf_TinhCong(@fromdate,@todate,@SoNgaySauKhiMangThaiDuocHuongCheDoThaiSan,
							  @fact,@Dept,@Sect,@Team,@Pos,@PosC,@Emp) tc
		LEFT JOIN HR_Shifts s
			   ON tc.ShiftName = s.ShiftName
		...
		JOIN      udf_EmployeeFilter('VN',@fact,@dept,@sect,@team,@pos,@posc,@Emp,@todate) ef
			   ON tc.Employee_ID = ef.Employee_ID
		LEFT JOIN HR_GoOut gout
			   ON tc.Employee_ID = gout.Employee_ID AND tc.AccessDate = gout.TimeDate
		LEFT JOIN HR_EmpRegisParameter erp
			   ON tc.Employee_ID = erp.Employee_ID 
			  AND tc.AccessDate BETWEEN erp.Fromdate AND erp.Todate
			  AND erp.Parameter = 'DiMuon' AND erp.ParameterValue = 'DiMuonSoPhut'
		LEFT JOIN CalFlag cf
			   ON CAST(tc.TimeDate AS date) = cf.Ngay
		WHERE (ef.TernimationDate IS NULL OR ef.TernimationDate >= @fromdate)
		  AND  ef.StartedDate <= @todate
		OPTION (RECOMPILE);

		CREATE NONCLUSTERED INDEX IX_#SRC ON #SRC(Employee_ID, TimeDate, AccessTime);

		IF EXISTS (SELECT 1 FROM tempdb.sys.indexes WHERE name='IX_#SRC' AND object_id = OBJECT_ID('tempdb..#SRC'))
		DROP INDEX IX_#SRC ON #SRC;

		-- Tạo CLUSTERED INDEX đúng thứ tự cần duyệt
		CREATE CLUSTERED INDEX CIX_#SRC ON #SRC(Employee_ID, TimeDate, AccessTime);
		
		-- Chèn sentinel cuối cùng (tránh UNION ALL)
		INSERT INTO #SRC(
		  Employee_ID, LeaveType_ID, TimeDate, AccessTime, InOutStatus, ShiftName,
		  maxovertime, MaxOverTimeBefore, MaxOverTimeHol, MaxOverTimeLunch, CheDo, HolSunTypeOfOT,
		  FromTime, ToTime, RestTimeFrom, RestTimeTo, AllowLateIn, AllowEarlyOut, MinMinute,
		  StartedDate, GoutTimeIn, GoutTimeOut, GoutLeaveType, ChoPhepSoPhutMuon,
		  IsSunday, IsHol50, IsHol99
		)
		VALUES ('ws900000000', NULL, GETDATE(), GETDATE(), NULL,NULL, NULL,0,0,0,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL, 0,0,0);

		DECLARE cur CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
		SELECT Employee_ID, LeaveType_ID, TimeDate, AccessTime, InOutStatus, ShiftName,
			   maxovertime, MaxOverTimeBefore, MaxOverTimeHol, MaxOverTimeLunch, CheDo, HolSunTypeOfOT,
			   FromTime, ToTime, RestTimeFrom, RestTimeTo, AllowLateIn, AllowEarlyOut, MinMinute,
			   StartedDate, GoutTimeIn, GoutTimeOut, GoutLeaveType, ChoPhepSoPhutMuon,
			   IsSunday, IsHol50, IsHol99
		FROM #SRC WITH (INDEX(CIX_#SRC)); -- đã có thứ tự cần
		OPEN  cur;
		...
		BEGIN
			if @OldEmployee_ID is null or @FirstAccessTime is null begin
				...
				------------------TÍNH CÔNG----------------------
					...
					if isnull(@OldCheDo,0)>=1 begin
						set @ShiftToTime= case when DATEPART(hour,@OldShiftTotime)>DATEPART(hour,@OldShiftFromTime) then dateadd(Hour,-1,@OldShiftTotime) else dateadd(Hour,-1,@OldShiftTotime)+1 end
					end else begin
						set @ShiftToTime= case when DATEPART(hour,@OldShiftTotime)>DATEPART(hour,@OldShiftFromTime) then @OldShiftTotime else @OldShiftTotime+1 end
					end
					...
					if not (@OldEmployee_ID=@OldInputEmployee_ID and @LastAccessTime<=@OldInputTimeOut) begin
						...
						if datediff(minute,@FirstAccessTime,@LastAccessTime)>=isnull(@MinMinute,15) begin
							...
							if @OldShiftRestFromTime is not null and @OldShiftRestToTime is not null begin
								if DATEPART(HOUR,@ShiftFromTime)<=DATEPART(HOUR,@OldShiftRestFromTime) begin
									set @ShiftRestFromTime= @OldShiftRestFromTime
								end else begin
									set @ShiftRestFromTime= @OldShiftRestFromTime + 1
								end
								if DATEPART(HOUR,@ShiftFromTime)<=DATEPART(HOUR,@OldShiftRestToTime) begin
									set @ShiftRestToTime= @OldShiftRestToTime
								end else begin
									set @ShiftRestToTime= @OldShiftRestToTime+1
								end
							end else begin
						...
						--print convert(nvarchar(MAX), @LastAccessTimeEditted, 20)
							...
							if (@OldIsSunday = 1) or @OldIsHol50 = 1 begin
								--xử lý loại tăng ca chủ nhât, ngày lễ
								if @OldIsSunday = 1 begin
							...
							--print @OldCheDo
							--Declare @float float = (case when @OldIsSunday = 1 or (@OldIsHol50 = 1 OR @OldIsHol99 = 1) then 0 else 1 end)
							--print @float
							if @LastAccessTimeEditted>@FirstAccessTimeEditted begin
								if isnull(@OldCheDo,0)>=1 begin
									insert into HR_WTDAILY (Employee_ID,Ngay,MaCong,wt,InsertSource) values(@OldEmployee_ID,@OldTimeDate, 'wt1' ,case when @OldIsSunday = 1 or (@OldIsHol50 = 1 OR @OldIsHol99 = 1) then 0 else 1 end,'TS')
								end
								...
								if	@FirstAccessTimeEditted<=dateadd(minute,-30,@ShiftRestToTime) and @LastAccessTimeEditted>=@ShiftRestToTime and @OldmaxovertimeLunch>0 begin
									if @OldShiftName not like '%Shift3' begin
										insert into HR_WTDAILY (Employee_ID,Ngay,MaCong,wt,InsertSource) values(@OldEmployee_ID,@OldTimeDate,case when @OldIsHol50 = 1 then 'CN_wt7' when @OldIsSunday = 1 or @OldIsHol99 = 1 then 'CN_wt4' else 'CN_wt3' end,@OldmaxovertimeLunch,'GiuaGio')
									end else begin
										insert into HR_WTDAILY (Employee_ID,Ngay,MaCong,wt,InsertSource) values(@OldEmployee_ID,@OldTimeDate,case when @OldIsHol50 = 1 then 'CN_wt8' when @OldIsSunday = 1 or @OldIsHol99 = 1 then 'CN_wt6' else 'CN_wt5' end,@OldmaxovertimeLunch,'GiuaGio')
									end
								end
								...
								*/
								...
								/* --- Block tối ưu trong vòng lặp: TÍNH WT chi tiết & nạp #WT_TMP --- */
								TRUNCATE TABLE #WT_TMP;

								;WITH S AS
								(
									SELECT rth.*
									FROM dbo.udf_ReturnTableSetupHourTimeKeeping
									(
										@OldEmployee_ID, @OldShiftName, @OldTimeDate,
										/* CheDo đặc thù: nếu có chế độ & có đơn nghỉ thì đẩy về 0 như logic cũ */
										CASE WHEN ISNULL(@OldCheDo,0) > 0 AND ISNULL(@OldLeaveType_ID,'0') <> '0' THEN 0 ELSE @OldCheDo END,
										CASE WHEN @GioTangCaToiDaTheoNgay < 0 THEN 0 ELSE @GioTangCaToiDaTheoNgay END
									) AS rth
									WHERE rth.FromTime < @LastAccessTimeEditted
									  AND rth.ToTime   > @FirstAccessTimeEditted
								)
								INSERT INTO #WT_TMP (Employee_ID, Ngay, MaCong, wt, InsertSource)
								SELECT
									@OldEmployee_ID,
									@OldTimeDate,
									S.MaCong,
									/* Quy đổi về WT theo công thức gốc của bạn */
									(
										CA.WTMin
										/ (CASE WHEN ISNULL(S.SoPhutCoSoDeQuyDoi,0)=0
												THEN (CASE WHEN ISNULL(S.SoPhutDuocTinhLaMotNgayCong,0)=0 THEN 480 ELSE S.SoPhutDuocTinhLaMotNgayCong END)
												ELSE S.SoPhutCoSoDeQuyDoi END)
									)
									* (CASE WHEN ISNULL(S.SoPhutDuocTinhLaMotNgayCong,0)=0 THEN 480 ELSE S.SoPhutDuocTinhLaMotNgayCong END) AS wt,
									/* InsertSource: TS/HolSun + HHmm + No_ + (TCT) */
									(CASE WHEN ISNULL(@OldCheDo,0) >= 1 THEN 'TS' ELSE '' END)
									+ ISNULL(@InertSourceHolSun,'')
									+ RIGHT('0000' + REPLACE(CONVERT(varchar(5), S.FromTime, 108), ':', ''), 4)
									+ CAST(S.No_ AS varchar(10))
									+ CASE WHEN S.isTCTruoc = 1 THEN 'TCT' ELSE '' END
								FROM S
								CROSS APPLY dbo.udf_TinhGioCongChiTiet_i
								(
									@FirstAccessTimeEditted,    -- @TimeIn
									@LastAccessTimeEditted,     -- @TimeOut
									S.FromTime,                 -- @ShiftFrom
									S.ToTime,                   -- @ShiftTo
									S.RestTimeFrom,             -- @RestTimeFrom
									S.RestTimeTo,               -- @RestTimeTo
									NULL,                       -- @isPushWorkingTime (giữ NULL như trước)
									S.MinMinute,                -- @MinMinute
									S.BlockMinute,              -- @BlockMinute
									S.BlockDownMinute,          -- @BlockDownMinute
									S.Round_,                   -- @Round
									@OldTimeDate,               -- @WorkingDay
									S.NumberOfDay,              -- @NumberOfDay
									S.MaCong                    -- @MaCong
								) AS CA
								OPTION (RECOMPILE);
								set @TongGioTangCa=0
								
								--select *, @FirstAccessTimeEditted, @LastAccessTimeEditted, @ShiftFromTime, @ShiftToTime from #WT_TMP

								select @TongGioTangCa=@TongGioTangCa+wt, @GioTangCaToiDaTheoNam=@GioTangCaToiDaTheoNam-wt,@GioTangCaToiDaTheoThang=@GioTangCaToiDaTheoThang-wt from #WT_TMP where macong in (select MaCong from HR_LoaiCong where isnull(isWorkingTime,0)=0 and MaCong not like 'CN%')
								
								-- XỬ LÝ NGƯỜI MỚI VÀO
								set @CongHCNguoiMoi=0
								if @OldStartedDate=@OldTimeDate begin
									select @CongHCNguoiMoi=@CongHCNguoiMoi+wt,@macong=macong from #WT_TMP where MaCong in (select macong from hr_loaicong where isnull(isWorkingTime,0)=1)
									if @CongHCNguoiMoi<8 begin
										insert into #WT_TMP (Employee_ID,Ngay,MaCong,wt,InsertSource) values(@OldEmployee_ID,@OldTimeDate,@macong,8-@CongHCNguoiMoi,case when isnull(@OldCheDo,0) >= 1 then 'TS' else '' end + 'NguoiMoi')
									end
								end
								-- END
								--select [dbo].[udf_TraVeLoaiNgayCong](@OldEmployee_ID,Ngay), @Employee_ID, Ngay, @OldEmployee_ID,@OldShiftName,@OldTimeDate,case when isnull(@OldCheDo,0) > 0 and isnull(@OldLeaveType_ID,'0') <> '0' then 0 else @OldCheDo end,(case when @GioTangCaToiDaTheoNgay<0 then 0 else @GioTangCaToiDaTheoNgay end), * from #WT_TMP
								...
								if @OldShiftName = '02-Shift2' begin
									if cast(@FirstAccessTimeEditted as time) <= '17:34:00.000' and (cast(@LastAccessTimeEditted as time) >= '23:00:00.000' or cast(@LastAccessTimeEditted as time) <= '05:00:00.000') begin
										delete #WT_TMP where MaCong in ('wt1','wt9')
										insert into #WT_TMP (Employee_ID,Ngay,MaCong,wt,InsertSource)
			...
			SET @OldIsSunday = @IsSunday;
			SET @OldIsHol50  = @IsHol50;
			SET @OldIsHol99  = @IsHol99;
		FETCH NEXT FROM cur INTO @Employee_ID,@LeaveType_ID,@TimeDate,@AccessTime,@InOutStatus,@ShiftName,@MaxOverTime,@MaxOverTimeBefore,@MaxOverTimeHol,@maxovertimeLunch,@CheDo,@HolSunTypeOfOT,@ShiftFromTime,@ShiftToTime,@ShiftRestFromTime,@ShiftRestToTime,@AllowLateIn,@AllowEarlyOut,@MinMinute,@StartedDate,@GoutTimeIn,@GoutTimeOut,@GoutLeaveType,@SoPhutChoDiMuon, @IsSunday, @IsHol50, @IsHol99
		END
		CLOSE cur
		DEALLOCATE cur
