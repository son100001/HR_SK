...
BEGIN
	...
	-- ================================================================================
	-- TẠO MỚI (độc lập, không đụng đơn khác) vs SỬA (đúng dòng theo ID) — xem ghi chú đầu file
	-- HR_RequestSave_SplitInsertUpdate.sql.
	--
	-- Hai điều kiện chặn khác nhau, xem ghi chú đầu file HR_RequestLeave_OverlapCheck.sql:
	--   1. Fromdate TRÙNG Y HỆT đơn khác (bất kể TrangThai, kể cả Rejected) — chặn CỨNG, vì khóa
	--      chính thật của bảng là (Employee_ID, Fromdate). Không chặn ở đây thì SQL Server tự chặn
	--      bằng lỗi PRIMARY KEY constraint thô, không dịch được ra thông báo cho người dùng.
	--   2. Overlap khoảng ngày (Fromdate khác nhau nhưng [Fromdate,ToDate] chồng lấn) — chỉ chặn
	--      "mềm" khi đơn cũ đang Pending/Approved; đơn cũ đã Rejected coi như đã đóng, không chặn.
	-- ================================================================================
	if isnull(@ThongBao,'')=''
	BEGIN
		IF @ID IS NOT NULL
		BEGIN
			IF NOT EXISTS (SELECT 1 FROM HR_EmployeeLeaveRequests WHERE ID = @ID)
				SET @ThongBao = N'RequestNotFound'
			ELSE IF EXISTS (
				SELECT 1 FROM HR_EmployeeLeaveRequests other
				WHERE other.Employee_ID = @Employee_ID
				  AND other.ID <> @ID
				  AND @Fromdate <= other.ToDate
				  AND @ToDate >= other.Fromdate
			)
				SET @ThongBao = N'LeaveRequestDateAlreadyExists'
			ELSE
			BEGIN
