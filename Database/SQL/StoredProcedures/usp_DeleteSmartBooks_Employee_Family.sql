CREATE PROCEDURE [dbo].[usp_DeleteSmartBooks_Employee_Family]
...
BEGIN
	...
	set @ThongBao=''
	select @Block_Date=Block_Date from HR_Khoa where [TableName]='SmartBooks_Employee_Family' and Block_User=@UserName
	...
	if @ThongBao='' begin
		if @Block_Date<@DependFromMonth or @Block_Date is null begin
			delete SmartBooks_Employee_Family where Employee_ID=@Employee_ID and RelatedName=@RelatedName and BirthDate=@BirthDate
		end else begin
