...
-- =============================================
--exec [dbo].[sp_GetSalaryComponentLongTerm] '2017-7-1','2017-7-31'
CREATE PROCEDURE [dbo].[sp_GetSalaryComponentLongTerm]
...
BEGIN
	...
	select distinct Employee_ID from [dbo].[udf_GetSalaryComponentNewest](@fromdate,@todate)
	DECLARE cur_SalaryComponentType CURSOR FOR
	select SalaryComponent from HR_SalaryComponentCategory where SalaryComponent in (select SalaryComponent from [dbo].[HR_SalaryComponentCategory] where [MonthlyChanging]=0 or [MonthlyChanging] is null) order by SalaryComponent
	...
	BEGIN
		...
		select Employee_ID,isnull(Amount,0) from [dbo].[udf_GetSalaryComponentNewest](@fromdate,@todate) where SalaryComponent=@SalaryComponent
		...
		BEGIN
			...
			exec ('update #tabSalaryComponent set '+@SalaryComponent+'='+@ValueConvert+' where Employee_ID=N'''+@Employee_ID+'''')
		FETCH NEXT FROM cur_SalaryComponent INTO @Employee_ID,@Value
		END
