--exec [sp_TinhGioXinRaNgoai] '2026-03-10','2026-03-10','admin',null,null,null,null,null,null,'C5310'
CREATE PROCEDURE [dbo].[sp_TinhGioXinRaNgoai]
...
BEGIN
	...
	BEGIN
		...
		--select @TimeOut, @timein, @FirstTimeIn, @TimeOut, @ShiftName
		...
		--if not (@TimeOut>=@LastTimeOut or @TimeIn<=@FirstTimeIn) begin
			...
			,[dbo].[udf_TinhGioCongChiTiet](@TimeOut,@TimeIn,FromTime,ToTime,RestTimeFrom,resttimeto
											,NULL,0,30,25,Round_,@WorkingDay,NumberOfDay,macong) as WorkingTime,'GOOUT_'+LEFT(REPLACE(convert(varchar, @TimeOut, 108),':',''),4)
			...
			if @LeaveType_ID = 'Business' begin
				if (@TimeIn not between @RealTimeIn and @RealTimeOut) or (@TimeOut not between @RealTimeIn and @RealTimeOut) begin
					update @HR_WTDAILY set wt=round((cast(wt*60 as float)/30*30/*-(case when cast(wt*60 as int)%30 between 1 and 29 then 30 else 0 end)*/)/60.0,2)
				end else begin
					FETCH NEXT FROM cur INTO @Employee_ID,@WorkingDay,@TimeOut,@TimeIn,@CheDo,@ShiftName,@ShiftFromTime,@ShiftToTime,@AllowLateIn,@MinMinute,@LeaveType_ID,@RealTimeIn,@RealTimeOut
					Continue
				end
