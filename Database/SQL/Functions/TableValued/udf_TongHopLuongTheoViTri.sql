CREATE function [dbo].[udf_TongHopLuongTheoViTri]
--select * from udf_TongHopLuongTheoViTri (1,2023,null,null,null,null,null,null,null,'MonthlySalary')
(
	@Month int,
	@Year int,
...
begin
	...
	insert into @rtnTongHopLuongTheoViTri
	select isnull(@MaCongViec,'All') as MaCongViec, isnull(empl.GroupByViTri,100) as GroupByViTri, count(s.Employee_ID) as SoLuongNV, sum(isnull(s.s1,0)) as BasicSalary, sum(isnull(s.s2,0)) + sum(isnull(s.s3,0)) + sum(isnull(s.s4,0)) + sum(isnull(s.s5,0)) + sum(isnull(s.s6,0)) + sum(isnull(s.s7,0)) + sum(isnull(s.s8,0)) + sum(isnull(s.s9,0)) + sum(isnull(s.s10,0)) + sum(isnull(s.s11,0)) + sum(isnull(s.s12,0)) as PhuCap, sum(isnull(s.s38,0)) as TienTangCa, sum(isnull(s.TongThuNhap,0))/(sum(isnull(s.s80,0)) - sum(isnull(s.s67,0)) - sum(isnull(s.s69,0))) as Luong1Gio, (sum(isnull(s.s17,0)) + sum(isnull(s.s19,0)))*8 - sum(isnull(s.s67,0)) - sum(isnull(s.s69,0)) as GioLamViecKhongCoNghi
	...
	where s.[key] = @SalaryKey and s.Salary_Month = @Month and s.Salary_Year = @Year and (case when isnull(@MaCongViec,'') = '' then '' else empl.CongViecPhaiLam end) = (case when isnull(@MaCongViec,'') = '' then '' else @MaCongViec end) and ChucDanh is not null
	group by case when @MaCongViec is null then 'All' else empl.CongViecPhaiLam end, empl.GroupByViTri
