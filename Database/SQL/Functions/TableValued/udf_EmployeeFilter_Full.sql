CREATE FUNCTION [dbo].[udf_EmployeeFilter_Full]
(
	-- Add the parameters for the function here
	--select * from udf_EmployeeFilter_Full ('VN',null,null,null,null,null,null,'C6745',GETDATE())
...
BEGIN
	...
	(
		...
		,empl.[BirthPlace]
		,(case when empl.Sex='Male' then N'Nam' when empl.Sex = 'Female' then N'Nữ' else N'' end) as Gender
	...
	where
	 (case when @fact is null or @fact='' then '' else empl.Factory_ID end) in (select data from [dbo].[Split](case when @fact is null or @fact='' then '' else @fact end,','))
			...
			and (
				...
				--and (empl.TernimationDate is null or empl.TernimationDate >= @NgayDauThang)
