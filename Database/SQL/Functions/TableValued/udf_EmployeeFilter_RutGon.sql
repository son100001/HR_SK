CREATE FUNCTION [dbo].[udf_EmployeeFilter_RutGon]
(
...
BEGIN
	...
	--, isnull(posc.AnnualLeaveDays
	--,isnull(pos.AnnualLeaveDays,isnull(team.AnnualLeaveDays,isnull(sect.AnnualLeaveDays,isnull(dept.AnnualLeaveDays,isnull(f.AnnualLeaveDays,0)))))) as NgayPhepNam
	--,f.Factory_ID as KhuVuc
	, cd.NameKR as isTrucTiep
	, dept.JobCode as ChoPhepSoPhutMuon, dept.DepartmentCode as DepartmentCode1, dept.JobCode as DepartmentGroup
	--, ins.BookCode as SoSoBaoHiem
	,cd.NameEN as LvDuyet--, empl.LuongThoaThuan
	,isnull(dept.OrderBy,9999) as OrderBy
	from 
	(
		...
		,empl.[ID_place], empl.NgayHetHanCCCD,empl.[Sex]
		--,empl.[MaritalStatus]
  ...
  --      CHARINDEX('-', REVERSE(empl.Address_Temporary) + '-') - 1))) AS Tinh_TP_TT,
		...
		,isnull(tf.SectionCode, empl.SectionCode) as SectionCode
		,tf.TeamCode
		...
		,isnull(tf.[ChucDanh],isnull(empl.ChucDanh,isnull(tf.[PositionCategory_ID],empl.[PositionCategory_ID]))) as [ChucDanh]
		--,tfEffectiveDate.EffectiveDateOfChucDanh
		,tf.[JobCode]
		--,empl.[Card_No],empl.[Card_Code],boe.[BankName],isnull(empl.BankCode,boe.[BankAccount]) as BankAccount,empl.[Tel],empl.BankCode,empl.[DebitAccount],empl.[Qualification]
		,isnull(empl.OfficialDate,hdct.NgayKyHDChinhThuc) as OfficialDate
		--,empl.[Graduated],empl.[GraduatedFrom],empl.[Email],empl.[MaSoThue],empl.[SoNgayPhepNam],empl.[ContractFlow],CASE WHEN TonGiao IS NULL THEN N'Không' ELSE TonGiao END AS TonGiao,empl.[NguoiLienHeGap],empl.Height,empl.[Weight],empl.Hospital,empl.HealthCheckFee,empl.[Remark],empl.[UserName],empl.[InsertDate]
		...
		--,empl.TypeOfHiring
		, empl.isManager
		--, empl.isTanTat, empl.Tantat, empl.CCCDCu, empl.CCCDCu_Date, empl.CCCDCu_Place
