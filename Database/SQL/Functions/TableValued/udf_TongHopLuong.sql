CREATE function [dbo].[udf_TongHopLuong] 
--select * from udf_TongHopLuong (1,2023,null,null,null,null,null,null,null,'MonthlySalary')
(
	@Month int,
	@Year int,
...
returns @rtnTongHopLuong table
(
	Salary_Month int, Salary_Year int, SoLuongNV int, BasicSalary float, PhuCap float, TienLuongHC float, TienTangCa float, Luong1Gio float, GioLamViecKhongCoNghi float, GioTangCa150 float, TienTangCa150 float, GioTangCaCN200 float, TienTangCaCN200 float, GioTangCaDem210 float, TienTangCaDem210 float, GioTangCaDemCN270 float, TienTangCaDemCN270 float, GioTangCaLe300 float, TienTangCaLe300 float, GioTangCaDemLe390 float, TienTangCaDemLe390 float, TongTangCa float, TongTangCaVaHCDaChuyen float, GioLamDem130 float, TienCaDem130 float, TongLuong float, CongDoanCtyTra float, BHCtyTra float, TongChiPhi float, BHNVDong float, TTCN float, CongDoanNVDong float, TroCapConNho float, TroCapPhuNu float, HoTroKhac float, ThucNhan float, LuongChoViec float, SoNVDongBH float, SoTienDongBH float, SoNguoiDongThue float, SoTienDongThue float, SoNguoiDongCongDoan float, SoTienDongCongDoan float, primary key (Salary_Month,Salary_Year)
)
as
begin
	...
	from
	SmartBooks_Salary s
	where s.[key] = @SalaryKey and s.Salary_Month = @Month and s.Salary_Year = @Year
	group by s.Salary_Year, s.Salary_Month
	return
end
GO
