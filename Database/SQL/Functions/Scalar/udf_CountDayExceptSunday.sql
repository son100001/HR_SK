CREATE FUNCTION [dbo].[udf_CountDayExceptSunday](@Fromdate datetime, @ToDate datetime)
...
as
	 begin  
		...
		while @Date <= @ToDate
		begin
			if Upper(left(datename(dw,@Date),3)) <> 'SUN' --and datediff(day,@Thu7DuocNghi,@Date) % 14 <> 0
			begin
			set @Count = @Count + 1
			end
			set @Date = @Date + 1
